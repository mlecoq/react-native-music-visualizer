    ...
    <Sheet visible={visible} onClose={close} title="Export video">
      {(phase === 'idle' || busy) && (
        <>
          <View className="mb-5 flex-row items-center gap-3 rounded-2xl border border-border bg-surface-2 p-4">
            <Icon name="film-line" size={22} color="#2DD4BF" />
            <View className="flex-1">
              <Text>1080 × 1920 · 30 fps · with the track's audio</Text>
          ...
          {busy ? (
            ...
            <Button onPress={start}>
              <Icon name="download-2-line" size={18} color="#05201C" />
              <Text>Export</Text>
            </Button>
          )}
        </>
      ...
      {phase === 'done' && (
        <View className="items-center py-2">
          <Icon name="checkbox-circle-fill" size={52} color="#2DD4BF" />
